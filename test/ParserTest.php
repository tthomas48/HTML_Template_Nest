...
class HTML_Template_Nest_ParserTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testSimpleTokens()
    {
        ...
        $this->assertEquals("<?php echo \$_o(\$p, 'some_var')?>", $output);
        $parser->registerVariable('my_var');
        $output = $parser->parse('${my_var}');
        $this->assertEquals("<?php echo \$_os(\$my_var)?>", $output);
    }
    ...
    public function testObjectVariables()
    {
        ...
        $this->assertEquals("<?php echo \$_o(\$p, 'some_var')->foo?>", $output);
        $output = $parser->parse('${some_var->FOO}');
        $this->assertEquals("<?php echo \$_o(\$p, 'some_var')->FOO?>", $output);
        $output = $parser->parse('${some_var->foo(bar,bin, baz)}');
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var')->foo(\$_o(\$p, 'bar'),\$_o(\$p, 'bin'),\$_o(\$p, 'baz'))?>", 
            $output
        );
        ...
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var')->foo(\$_o(\$p, 'bar')," .
        ...
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var')->foo(\$_o(\$p, 'bar'),\"bin\"" .
    ...
    public function testArrays()
    {
        ...
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var')[0]?>", 
            $output
        );
        $output = $parser->parse('${some_var["value"]}');
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var')[\"value\"]?>", 
            $output
        );        
        $output = $parser->parse('${some_var["value"][0]}');
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var')[\"value\"][0]?>", 
            $output
        );        
    }
    ...
    public function testOperators()
    {
        ...
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var') + \$_o(\$p, 'some_other_var')?>", 
            $output
        );
        $output = $parser->parse('${some_var < some_other_var}');
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var') < \$_o(\$p, 'some_other_var')?>", $output
        );
        $output = $parser->parse('${some_var <= some_other_var}');
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var') <= \$_o(\$p, 'some_other_var')?>", $output
        ...
        $this->assertEquals(
            "<?php echo (\$_o(\$p, 'some_var') % \$_o(\$p, 'some_other_var')) == 0?>", $output
        );
        $output = $parser->parse('${some_var / some_other_var}');
        $this->assertEquals(
            "<?php echo \$_o(\$p, 'some_var') / \$_o(\$p, 'some_other_var')?>", $output
        ...
        $this->assertEquals(
            "<?php echo (\$_o(\$p, 'a') + \$_o(\$p, 'b') + \$_o(\$p, 'c')) * (\$_o(\$p, 'd') - 3)?>", 
            $output
        );
        $output = $parser->parse('${(a + b + c) && (d - 3)}');
        $this->assertEquals(
        ...
        $this->assertEquals("<?php echo \$_o(\$p, 'foo') == 'a'?>", $output);
        $output = $parser->parse('${foo == "b"}');
        $this->assertEquals("<?php echo \$_o(\$p, 'foo') == \"b\"?>", $output);        
        
        $output = $parser->parse('${(foo == "b" ? "black" : "red")}');
        $this->assertEquals(
            "<?php echo (\$_o(\$p, 'foo') == \"b\" ? \"black\" : \"red\")?>", 
            $output
        );        
    }
    ...
    public function testFunctions()
    {
        ...
        $this->assertEquals("<?php echo count(\$_o(\$p, 'myarray'))?>", $output);
        $output = $parser->parse('${fn:str_replace(\'foo\', "bar", myarray)}');
        $this->assertEquals(
            "<?php echo str_replace('foo',\"bar\",\$_o(\$p, 'myarray'))?>",
            $output
        );
